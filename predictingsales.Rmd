...
express <- express %>% filter(date >= "2014-07-10")

#split data
inTrain <- createDataPartition(y = express$sale, p=0.8)$Resample
...
express_ols <- train_set_express %>% lm(sale ~ ny_snow + ny_mean_temp + ny_precip + la_snow + la_mean_temp +la_precip + chi_snow + chi_mean_temp + chi_precip + hou_snow + hou_mean_temp + hou_precip + unemployment + expr_adj_close, data = .)
                                          
#fit glm logistic regression to train set
express_glm <- train_set_express %>% glm(sale ~ ny_snow + ny_mean_temp + ny_precip + la_snow + la_mean_temp +la_precip + chi_snow + chi_mean_temp + chi_precip + hou_snow + hou_mean_temp + hou_precip + unemployment + expr_adj_close, data=. , family = "binomial")
...
table(test_set_express$accuracy_express_glm)

#confusion matrix ols
tab <- table(test_set_express$pred_express_ols, test_set_express$sale)
...
```
OLS: 80.3% accuracy
GLM: 80.3% accuracy???? Why is it the same? Check on this!

GAP
```{r}
#filter for date of furthest tweet back
gap <- gap %>% filter(date >= "2015-02-12")

#split data
inTrain <- createDataPartition(y = gap$sale, p=0.8)$Resample
...
#fit OLS regression to train set
gap_ols <- train_set_gap %>% lm(sale ~ ny_snow + ny_mean_temp + ny_precip + la_snow + la_mean_temp +la_precip + chi_snow + chi_mean_temp + chi_precip + hou_snow + hou_mean_temp + hou_precip + unemployment + gap_adj_close, data=.)

#fit glm logistic regression to train set
gap_glm <- train_set_gap %>% glm(sale ~ ny_snow + ny_mean_temp + ny_precip + la_snow + la_mean_temp +la_precip + chi_snow + chi_mean_temp + chi_precip + hou_snow + hou_mean_temp + hou_precip + unemployment + gap_adj_close, data=. , family = "binomial")
...
LOFT
```{r}
loft <- loft %>% filter(date >= "2013-03-01")
...
#fit OLS regression to train set
loft_ols <- train_set_loft %>% lm(sale ~ ny_snow + ny_mean_temp + ny_precip + la_snow + la_mean_temp +la_precip + chi_snow + chi_mean_temp + chi_precip + hou_snow + hou_mean_temp + hou_precip + unemployment + loft_adj_close, data=.)

#fit glm logistic regression to train set
loft_glm <- train_set_loft %>% glm(sale ~ ny_snow + ny_mean_temp + ny_precip + la_snow + la_mean_temp +la_precip + chi_snow + chi_mean_temp + chi_precip + hou_snow + hou_mean_temp + hou_precip + unemployment + loft_adj_close, data=. , family = "binomial")
...
JCREW
```{r}
jcrew <- jcrew %>% filter(date >= "2013-09-25")
...
#fit OLS regression to train set
jcrew_ols <- train_set_jcrew %>% lm(sale ~ ny_snow + ny_mean_temp + ny_precip + la_snow + la_mean_temp +la_precip + chi_snow + chi_mean_temp + chi_precip + hou_snow + hou_mean_temp + hou_precip + unemployment, data=.)

#fit glm logistic regression to train set
jcrew_glm <- train_set_jcrew %>% glm(sale ~ ny_snow + ny_mean_temp + ny_precip + la_snow + la_mean_temp +la_precip + chi_snow + chi_mean_temp + chi_precip + hou_snow + hou_mean_temp + hou_precip + unemployment, data=. , family = "binomial")
